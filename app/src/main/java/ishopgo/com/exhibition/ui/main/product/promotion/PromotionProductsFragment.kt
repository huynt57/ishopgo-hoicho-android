...
class PromotionProductsFragment : BaseListFragment<List<Product>, Product>() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        (viewModel as PromotionProductsViewModel).loadRegion.observe(this, Observer { p ->
            p?.let {
                val region = Region()
        ...
        (viewModel as PromotionProductsViewModel).loadDistrict.observe(this, Observer { p ->
            p?.let {
        ...
        (viewModel as PromotionProductsViewModel).childCategories.observe(this, Observer { c ->
            c?.let {
                childAdapter.replaceAll(it)
            }
        })

        (viewModel as PromotionProductsViewModel).categories.observe(this, Observer { c ->
            c?.let {
        ...
        (viewModel as PromotionProductsViewModel).loadRegion()
        if (category.id > 0) {
            (viewModel as PromotionProductsViewModel).loadChildCategory(category)
            view_category.text = category.name ?: ""
        } else {
            view_category.text = "Chọn danh mục"
            (viewModel as PromotionProductsViewModel).loadCategories()
        }
    }
                ...
                override fun click(position: Int, data: Region, code: Int) {
                    context?.let {
                        ...
                        if (position == 0) {
                            ...
                            data.provinceid?.let { it1 -> (viewModel as PromotionProductsViewModel).loadDistrict(it1) }

...
class PopularFragment : BaseListFragment<List<Product>, Product>() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        (viewModel as PopularProductsViewModel).loadRegion.observe(this, Observer { p ->
            p?.let {
                val region = Region()
        ...
        (viewModel as PopularProductsViewModel).loadDistrict.observe(this, Observer { p ->
            p?.let {
        ...
        (viewModel as PopularProductsViewModel).childCategories.observe(this, Observer { c ->
            c?.let {
                childAdapter.replaceAll(it)
            }
        })

        (viewModel as PopularProductsViewModel).categories.observe(this, Observer { c ->
            c?.let {
        ...
        (viewModel as PopularProductsViewModel).loadRegion()
        if (category.id > 0) {
            (viewModel as PopularProductsViewModel).loadChildCategory(category)
            view_category.text = category.name ?: ""
        } else {
            view_category.text = "Chọn danh mục"
            (viewModel as PopularProductsViewModel).loadCategories()
        }
    }
                ...
                override fun click(position: Int, data: Region, code: Int) {
                    context?.let {
                        ...
                        if (position == 0) {
                            ...
                            data.provinceid?.let { it1 -> (viewModel as PopularProductsViewModel).loadDistrict(it1) }

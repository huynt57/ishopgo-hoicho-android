...
class MockAuthService(behavior: BehaviorDelegate<ApiService.Auth>) : ApiService.Auth {
    override fun updateStampDiaryInfo(id: Long, body: RequestBody): Single<BaseResponse<Any>> {
        val response = BaseResponse<Any>()
        response.status = 1
        return delegate.returningResponse(response).updateStampDiaryInfo(id, body)
    }
    ...
    override fun getStampCodeFromProduct(id: Long): Single<BaseResponse<List<CodeNoStamp>>> {
        val response = BaseResponse<Any>()
        response.status = 1
        return delegate.returningResponse(response).getStampCodeFromProduct(id)
    }
    ...
    override fun updateInfoFormFacebook(body: RequestBody): Single<BaseResponse<Profile>> {
        val response = BaseResponse<Any>()
        response.status = 1
        return delegate.returningResponse(response).updateInfoFormFacebook(body)
    }
